...
high_Neyman_ETEL<-function(seed,n){
  p = 500
  sig = array(0.3,c(p,p))
  diag(sig) = 1
  ...
  ci<-0
  postmean_theta1 <- mean(res$post_betel_pscov_ate)
...
lasso_Neyman_ETEL<-function(seed,n){
  p = 500
  sig = array(0.3,c(p,p))
  diag(sig) = 1
  expit = function(x) {1/(1+exp(-x))}
  ...
  dhat2 <- predict(fd2, x[samp, ],s=fy2$lambda.min,type="response")
  rd[samp] <- d[samp] - dhat2
...
nnet_Neyman_ETEL<-function(seed,n){
  p = 500
  sig = array(0.3,c(p,p))
  diag(sig) = 1
  expit = function(x) {1/(1+exp(-x))}
