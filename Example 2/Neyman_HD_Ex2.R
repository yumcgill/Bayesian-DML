...
high_Neyman_HD_ex2<-function(seed,n){
  p = 40
  sig = array(0.05,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
  d = rnorm(n, mean = 0.45*x[,1] + 0.9*x[,2] - 0.4*x[,5] +1.3*x[,2]*x[,5] ,sd = 1 )
  y = rnorm(n, mean= d + 0.5*x[,1] + x[,3] - 0.1*x[,4]-0.2*x[,7] + 1.5*x[,3]*x[,4], sd=1)
...
lasso_Neyman_HD_ex2<-function(seed,n){
  p = 40
  sig = array(0.05,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
  ...
  dhat2 <- predict(fd2, x[samp, ],s=fy1$lambda.min)
  rd[samp] <- d[samp] - dhat2
...
nnet_Neyman_HD_ex2<-function(seed,n){
  p = 40
  sig = array(0.1,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
  d = rnorm(n, mean = 0.45*x[,1] +x[,2]- 0.4*x[,5]  ,sd = 1 )
  y = rnorm(n, mean= d  + x[,3] - 0.1*x[,4]-0.2*x[,7], sd=1)
