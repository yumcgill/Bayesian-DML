### Algorithm 2 with K=2, Bayesian EL 
Neyman_EL<-function(Y_res,D_res){
  ...
  old.theta<-solveGel(update(emp_mod, gelType="EL"), theta0=old.theta,tControl=list(method="Brent", lower = -10, upper = 10))$theta
  ...
  fit <- gelFit(emp_mod,gelType="EL",theta0=old.theta,tControl=list(method="Brent", lower = -10, upper = 10))
  ...
  for (i in 2:nits){
    ...
    new.lam<-evalGel(emp_mod,para_p,gelType="EL",tControl=list(method="Brent", lower = -10, upper = 10))@lambda
    ## log emp likelihood value
    emp_log_lik_new<-sum(log(getImpProb(evalGel(emp_mod,para_p,new.lam,gelType="EL",tControl=list(method="Brent", lower = -10, upper = 10)))$pt))
  ...
  return(list(post_bEL_pscov_ate = post_ate))
} 

### Simulation EXAMPLE 2 


high_Neyman_EL_ex2<-function(seed,n){
  p = 40
  ...
  ci<-0
  postmean_theta1 <- mean(res$post_bEL_pscov_ate)
  if(quantile(res$post_bEL_pscov_ate,prob=c(0.025))<1 && quantile(res$post_bEL_pscov_ate,prob=c(0.975))>1) {ci<-1}
  return(list(theta=postmean_theta1,ci=ci,run_time=run_time))
}

lasso_Neyman_EL_ex2<-function(seed,n){
  p = 40
  sig = array(0.05,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
...
nnet_Neyman_EL_ex2<-function(seed,n){
  p = 40
  sig = array(0.05,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
