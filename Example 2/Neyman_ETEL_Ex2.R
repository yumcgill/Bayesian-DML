### Algorithm 2 with K=2, Bayesian ETEL 
Neyman_BETEL<-function(Y_res,D_res){
  ...
  old.theta<-solveGel(update(emp_mod, gelType="ETEL"), theta0=old.theta,tControl=list(method="Brent", lower = -10, upper = 10))$theta
  ...
  fit <- gelFit(emp_mod,gelType="ETEL",theta0=old.theta,tControl=list(method="Brent", lower = -10, upper = 10))
  ...
  for (i in 2:nits){
    ...
    new.lam<-evalGel(emp_mod,para_p,gelType="ETEL",tControl=list(method="Brent", lower = -10, upper = 10))@lambda
    ## log emp likelihood value
    emp_log_lik_new<-sum(log(getImpProb(evalGel(emp_mod,para_p,new.lam,gelType="ETEL",tControl=list(method="Brent", lower = -10, upper = 10)))$pt))
...
high_Neyman_ETEL_ex2<-function(seed,n){
  p = 40
  ...
  rd<- rep(0,n)
  ry <- rep(0,n)
  fy1 <- randomForest(x[samp, ], y[samp], clas_nodesize=3, reg_nodesize=5, ntree=1000,na.action=na.omit, replace=TRUE)
  yhat1 <- predict(fy1, x[osamp, ])
  ry[osamp] <- y[osamp] - yhat1
  
  fd1 <- randomForest(x[samp, ], d[samp],clas_nodesize=3, reg_nodesize=5, ntree=1000,na.action=na.omit, replace=TRUE)
  dhat1 <- predict(fd1, x[osamp, ])
  rd[osamp] <- d[osamp] - dhat1
  
  fy2 <- randomForest(x[osamp, ], y[osamp], clas_nodesize=3, reg_nodesize=5, ntree=1000,na.action=na.omit, replace=TRUE)
  yhat2 <- predict(fy2, x[samp, ])
  ry[samp] <- y[samp] - yhat2
  
  fd2 <- randomForest(x[osamp, ], d[osamp], clas_nodesize=3, reg_nodesize=5, ntree=1000,na.action=na.omit, replace=TRUE)
  dhat2 <- predict(fd2, x[samp, ])
  rd[samp] <- d[samp] - dhat2
  
  res<-Neyman_BETEL(ry,rd)
  
  ena_time<- Sys.time()
  run_time <- ena_time - start_time
  
  ci<-0
  postmean_theta1 <- mean(res$post_betel_pscov_ate,na.rm=TRUE)
  if(quantile(res$post_betel_pscov_ate,prob=c(0.025))<1 && quantile(res$post_betel_pscov_ate,prob=c(0.975))>1) {ci<-1}
  return(list(theta=postmean_theta1,ci=ci,run_time=run_time))
}

lasso_Neyman_ETEL_ex2<-function(seed,n){
  p = 40
  sig = array(0.05,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
  ...
  fd1 <- cv.glmnet(x[samp, ], d[samp],alpha = 1)
  dhat1 <- predict(fd1, x[osamp, ],s=fy1$lambda.min)
  rd[osamp] <- d[osamp] - dhat1
  ...
  fd2 <- cv.glmnet(x[osamp, ], d[osamp],alpha = 1)
  dhat2 <- predict(fd2, x[samp, ],s=fy2$lambda.min)
  rd[samp] <- d[samp] - dhat2
...
nnet_Neyman_ETEL_ex2<-function(seed,n){
  p = 40
  sig = array(0.05,c(p,p))
  diag(sig) = 1
  x = matrix(rmvnorm(n,mean=rep(0, nrow(sig)), sigma=sig),n,p)
  ...
  d.s <- scale(d, center = min(d), scale = max(d) - min(d))
  start_time<- Sys.time()
  samp <- sample(1:n, floor(n / 2))
  osamp <- setdiff(1:n, samp)
  rd<- rep(0,n)
  ...
  fd1 <- nnet(x.s[samp,],d.s[samp],size=2,  maxit=1000, decay=0.01, MaxNWts=10000,  trace=FALSE,linout = TRUE)
  dhat1 <- predict(fd1, x.s[osamp, ])*(max(d) - min(d))+min(d)
  rd[osamp] <- d[osamp] - dhat1
  ...
  fd2 <-  nnet(x.s[osamp,],d.s[osamp],size=2,  maxit=1000, decay=0.01, MaxNWts=10000,  trace=FALSE,linout = TRUE)
  dhat2 <- predict(fd2, x.s[samp, ])*(max(d) - min(d))+min(d)
  rd[samp] <- d[samp] - dhat2
